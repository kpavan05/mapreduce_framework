...
int main(int argc, char** argv) {
  ...
  framework->create()->withFlags(ZOO_EPHEMERAL)->forPath("/workers/worker1");
  std::vector<string> children = framework->getChildren()->forPath("/workers");
  std::cout<< "number of children " << children.size() << std::endl;

  framework->setData()->forPath("/workers/worker1", ipAddress.c_str() );
  std::string ipAddrData = framework->getData()->forPath("/workers/worker1");
  std::cout <<"ip address of worker" << ipAddrData << std::endl;
  LOG(INFO) << "ip address of worker" << ipAddrData;
  RunServer(ipAddress);
